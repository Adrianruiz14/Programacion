...
public class ej4 {
    ...
    public static void main(String[] args) {
        ...
        int fila = random.nextInt(10, 20);
        int columna = random.nextInt(10, 20);
        int[][] matriz;
        do {
...
public static void buscador(int[][] matriz) {
    ...
    // Empezamos en 1 y terminamos en length-1 para no salirnos de los bordes -------IMPORTANTE-----------
    // MARCO DE SEGURIDAD -> EVITAR LOS BORDES YA QUE COMPROBANDO EL ADYACENTE DEL K ESTÁ EN EL BORDE SE COMPRUEBA ESTE TAMBIÉN.
        ...
        for (int j = 1; j < matriz[i].length - 1; j++) {
            ...
            if (matriz[i][j] == 0) {
                ...
                if (matriz[i-1][j-1] == 1 && matriz[i-1][j] == 1 && matriz[i-1][j+1] == 1 &&
                    ...
                    encontrado = true;
                    return; // Paramos en la primera que encontre
                }
            }
        }
    }
